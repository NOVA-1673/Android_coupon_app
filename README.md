...
##진행 완료 상황 (2026.01.15 기준)

HCE(Host Card Emulation) 를 활용하여
사장폰을 고객폰에 태깅하면 NFC 통신을 통해 쿠폰 정보를 교환

사장폰에서 적립 확인 팝업 표시 → 확인 시

고객폰: 적립 완료 알림 및 쿠폰 +1 반영

사장폰: 적립 성공 결과 저장

앱이 실행 중이지 않은 상태에서도
HCE 기반 NFC 통신이 동작하도록 구현

##개선 예정 사항

고객폰에서 쿠폰 데이터 임의 변경 방지

HCE 응답 조건 강화

중복 적립 및 비정상 요청 차단 로직 추가 예정

실제 쿠폰 사용 흐름과 유사한 UI/UX 적용

실물 쿠폰 스탬프 카드 스타일

적립/사용 상태를 직관적으로 표현하는 화면 구성
